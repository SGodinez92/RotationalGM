...
pattern Plain $patternTagGravity "Linear" {
	for {set i 1} {$i <= $nStory} {incr i 1} {
		...
		# Column loads
		load [expr 1000*$i + 800 +  1] 0. 0. [expr -$w_tot*$ft*$ft*198.00] 0. 0. 0.
		load [expr 1000*$i + 800 +  2] 0. 0. [expr -$w_tot*$ft*$ft*576.00] 0. 0. 0.
		load [expr 1000*$i + 800 +  3] 0. 0. [expr -$w_tot*$ft*$ft*633.75] 0. 0. 0.
		load [expr 1000*$i + 800 +  4] 0. 0. [expr -$w_tot*$ft*$ft*255.75] 0. 0. 0.
		load [expr 1000*$i + 800 +  5] 0. 0. [expr -$w_tot*$ft*$ft*342.00] 0. 0. 0.
		load [expr 1000*$i + 800 +  6] 0. 0. [expr -$w_tot*$ft*$ft*576.75] 0. 0. 0.
		load [expr 1000*$i + 800 +  7] 0. 0. [expr -$w_tot*$ft*$ft*306.00] 0. 0. 0.
		load [expr 1000*$i + 800 +  8] 0. 0. [expr -$w_tot*$ft*$ft*530.25] 0. 0. 0.
		load [expr 1000*$i + 800 +  9] 0. 0. [expr -$w_tot*$ft*$ft*162.00] 0. 0. 0.
		load [expr 1000*$i + 800 + 10] 0. 0. [expr -$w_tot*$ft*$ft*351.00] 0. 0. 0.
		load [expr 1000*$i + 800 + 11] 0. 0. [expr -$w_tot*$ft*$ft*395.28] 0. 0. 0.
		load [expr 1000*$i + 800 + 12] 0. 0. [expr -$w_tot*$ft*$ft*209.25] 0. 0. 0.
		load [expr 1000*$i + 800 + 13] 0. 0. [expr -$w_tot*$ft*$ft*147.00] 0. 0. 0.
		load [expr 1000*$i + 800 + 14] 0. 0. [expr -$w_tot*$ft*$ft*147.00] 0. 0. 0.
		load [expr 1000*$i + 800 + 15] 0. 0. [expr -$w_tot*$ft*$ft*135.00] 0. 0. 0.
		load [expr 1000*$i + 800 + 16] 0. 0. [expr -$w_tot*$ft*$ft*135.00] 0. 0. 0.
	}
}
...
pattern Plain $patternTagGravity "Linear" {
	for {set i 1} {$i < [expr $nStory - 1]} {incr i} {
		...
		load [expr 1000*$i + 800 + 13] 0. 0. [expr -20.*$in*20.*$in*12.*$ft*150.*$pcf] 0. 0. 0.
		load [expr 1000*$i + 800 + 14] 0. 0. [expr -20.*$in*20.*$in*12.*$ft*150.*$pcf] 0. 0. 0.
		load [expr 1000*$i + 800 + 15] 0. 0. [expr -20.*$in*20.*$in*12.*$ft*150.*$pcf] 0. 0. 0.
		load [expr 1000*$i + 800 + 16] 0. 0. [expr -20.*$in*20.*$in*12.*$ft*150.*$pcf] 0. 0. 0.
