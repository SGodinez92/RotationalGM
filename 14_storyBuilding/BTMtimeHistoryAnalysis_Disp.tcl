...
set gm_dirctn_V 3;			# ground-motion direction (3 - Z)

#set gm_dirctn_RotX 4;
#set gm_dirctn_RotY 5;

set gm_dirctn_RotX 4;
set gm_dirctn_RotY 5;
set gm_dirctn_RotZ 6;

# set up ground-motion-analysis parameters

set gmDataFile_L [open "GroundMotions/$EQlevel/H1_D_$RSN.dat" "r"];
set gmDataFile_T [open "GroundMotions/$EQlevel/H2_D_$RSN.dat" "r"];
#set gmDataFile_V [open "GroundMotions/$EQlevel/H3_D_$RSN.dat" "r"];
#set gmDataFile_L [open "GroundMotions/$EQlevel/H1_$RSN.dat" "r"];
#set gmDataFile_T [open "GroundMotions/$EQlevel/H2_$RSN.dat" "r"];
#set gmDataFile_V [open "GroundMotions/$EQlevel/H3_$RSN.dat" "r"];

#set gmDataFile_RotY [open "GroundMotions/$EQlevel/RotY_D_$RSN_shift_015.dat" "r"];
#set gmDataFile_RotX [open "GroundMotions/$EQlevel/RotX_D_$RSN_shift_015.dat" "r"];

#set gmDataFile_RotX [open "GroundMotions/$EQlevel/ROT_H1_D.dat" "r"];
#set gmDataFile_RotY [open "GroundMotions/$EQlevel/ROT_H2_D.dat" "r"];
#set gmDataFile_RotZ [open "GroundMotions/$EQlevel/ROT_H3_D.dat" "r"];

set gmDataFile_RotX [open "GroundMotions/$EQlevel/ROT_H1_D_noShift.dat" "r"];
set gmDataFile_RotY [open "GroundMotions/$EQlevel/ROT_H2_D_noShift.dat" "r"];
set gmDataFile_RotZ [open "GroundMotions/$EQlevel/ROT_H3_D_noShift.dat" "r"];

set gmData_L [read $gmDataFile_L];
set gmData_T [read $gmDataFile_T];
#set gmData_V [read $gmDataFile_V];

#set gmData_RotY [read $gmDataFile_RotY];
#set gmData_RotX [read $gmDataFile_RotX];

set gmData_RotX [read $gmDataFile_RotX];
set gmData_RotY [read $gmDataFile_RotY];
set gmData_RotZ [read $gmDataFile_RotZ];

close $gmDataFile_L;
close $gmDataFile_T;
#close $gmDataFile_V;

#close $gmDataFile_RotY;
#close $gmDataFile_RotX;

close $gmDataFile_RotX;
close $gmDataFile_RotY;
close $gmDataFile_RotZ;

#set npts_Rot [lindex $gmData_RotY 0];
set npts_Rot [lindex $gmData_RotX 0];
#set npts_Rot [lindex $gmData_L 0];

set npts [expr $npts_Rot - 1]

#set dtRec_Rot [lindex $gmData_RotY 1];
set dtRec_Rot [lindex $gmData_RotX 1];
...
#set gmDataInput_V {}
#for {set i 1} {$i <= $npts} {incr i} {
#    lappend gmDataInput_V [lindex $gmData_V [expr $i + 1]]
#}
#
set gmDataInput_RotX {}
for {set i 1} {$i <= $npts} {incr i} {
    lappend gmDataInput_RotX [lindex $gmData_RotX [expr $i + 1]]
}

set gmDataInput_RotY {}
for {set i 1} {$i <= $npts} {incr i} {
    lappend gmDataInput_RotY [lindex $gmData_RotY [expr $i + 1]]
}

set gmDataInput_RotZ {}
for {set i 1} {$i <= $npts} {incr i} {
    lappend gmDataInput_RotZ [lindex $gmData_RotZ [expr $i + 1]]
}

#set gmDataInput_Rot {}
#for {set i 1} {$i <= $npts} {incr i} {
#    lappend gmDataInput_Rot [lindex $gmData_Rot [expr $i + 1]]
#}

#set percentageTime 1.0
set percentageTime 0.999
...
#set gmLoadTag_V $timeHisTag;	# LoadTag for Uniform ground motion excitation
#incr timeHisTag
#
#set tsTag_V $gmLoadTag_V;
#
set gmLoadTag_RotX $timeHisTag;	# LoadTag for Uniform ground motion excitation
incr timeHisTag

set tsTag_RotX $gmLoadTag_RotX;

set gmLoadTag_RotY $timeHisTag;	# LoadTag for Uniform ground motion excitation
incr timeHisTag

set tsTag_RotY $gmLoadTag_RotY;

set gmLoadTag_RotZ $timeHisTag;	# LoadTag for Uniform ground motion excitation
incr timeHisTag

set tsTag_RotZ $gmLoadTag_RotZ;

#set gmLoadTag_Rot $timeHisTag;	# LoadTag for Uniform ground motion excitation
#incr timeHisTag
#
#set tsTag_Rot $gmLoadTag_Rot;

timeSeries Path $tsTag_L -dt $dtRec -values $gmDataInput_L -factor 1.; # define displacement vector from file (displacement already in inches)
timeSeries Path $tsTag_T -dt $dtRec -values $gmDataInput_T -factor 1.; # define displacement vector from file (displacement already in inches)
#timeSeries Path $tsTag_V -dt $dtRec -values $gmDataInput_V -factor 1.; # define displacement vector from file (displacement already in inches)

timeSeries Path $tsTag_RotX -dt $dtRec -values $gmDataInput_RotX -factor 1.; # define displacement vector from file (rotation in radians)
timeSeries Path $tsTag_RotY -dt $dtRec -values $gmDataInput_RotY -factor 1.; # define displacement vector from file (rotation in radians)

#timeSeries Path $tsTag_Rot -dt $dtRec -values $gmDataInput_Rot -factor 1.; # define displacement vector from file (rotation in radians)
timeSeries Path $tsTag_RotZ -dt $dtRec -values $gmDataInput_RotZ -factor 1.; # define displacement vector from file (rotation in radians)

#pattern MultipleSupport $patternTag
pattern MultipleSupport $gmLoadTag_RotX {
	
	#groundMotion $gmTag Plain <-accel $tsTag> <-vel $tsTag> <-disp $tsTag> <-int (IntegratorType intArgs)> <-fact $cFactor>
	#groundMotion $gmLoadTag_L Plain -accel $tsTag_L
	#groundMotion $gmLoadTag_T Plain -accel $tsTag_T
	#groundMotion $gmLoadTag_V Plain -accel $tsTag_V
	groundMotion $gmLoadTag_L Plain -disp $tsTag_L
	groundMotion $gmLoadTag_T Plain -disp $tsTag_T
	#groundMotion $gmLoadTag_V Plain -disp $tsTag_V
	
	groundMotion $gmLoadTag_RotX Plain -disp $tsTag_RotX
	groundMotion $gmLoadTag_RotY Plain -disp $tsTag_RotY
	
	#groundMotion $gmLoadTag_Rot Plain -disp $tsTag_Rot
	groundMotion $gmLoadTag_RotZ Plain -disp $tsTag_RotZ
	
	#imposedMotion $nodeTag $dirn $gMotionTag
	imposedMotion 1 $gm_dirctn_L $gmLoadTag_L
	imposedMotion 1 $gm_dirctn_T $gmLoadTag_T
	#imposedMotion 1 $gm_dirctn_V $gmLoadTag_V
	
	imposedMotion 1 $gm_dirctn_RotX $gmLoadTag_RotX
	imposedMotion 1 $gm_dirctn_RotY $gmLoadTag_RotY
	
	#imposedMotion 1 $gm_dirctn_Rot $gmLoadTag_Rot
	imposedMotion 1 $gm_dirctn_RotZ $gmLoadTag_RotZ
}
