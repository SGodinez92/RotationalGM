...
# RectangularWalls
proc nodesCompY {xoComp yoComp zoComp nIncCShape nIncCShapeZ story coreWallTag incCshape nIncPierZ incPierZ nIncCShapeZbelow recInput} {
#set region_node {}
set nodeNum 1
for {set verAxis 1} {$verAxis <= [expr {$nIncCShape + 1}]} {incr verAxis} {
  for {set horAxis 1} {$horAxis <= [expr {$nIncCShapeZ + 1}]} {incr horAxis} {
	...
	set coordx $xoComp
    set coordy [expr {$yoComp + ($verAxis-1)*$incCshape}]
...
set nodeTagS [expr $coreWallTag + $nIncPierZ + 1]
set nodeTagN [expr $coreWallTag + ($nIncPierZ + 1)*($nIncCShape+1)]
...
append Nodes { } [nodesCompY $xoComp $yoComp $zoComp $nIncCShape $nIncCShapeZ $story $coreWallTag $incCshape $nIncPierZ $incPierZ $nIncCShapeZbelow $recInput]
puts $ModelFile "node $nodeTagS $xoComp $yoComp $coordzSPM"
puts $ModelFile "node $nodeTagN $xoComp [expr $yoComp+$lCshape] $coordzSPM"
