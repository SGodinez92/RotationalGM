...
#Corners
set AcornE [expr {144.*288.}];	#East corner

set PcornE [expr $w_tot*$AcornE]
...
# loads corners
set P_cx_cornE [expr {$PcornE + $LtribSlab_cx_corn*$wl_c + $LtribSelf_cx_corn*$wself_wall}]
set P_cx_cornW $P_cx_cornE
...
pattern Plain $patternTag "Linear" { 
	...
	load [expr {$coreWallTag + ($nIncCShapeZ+1)}] 0 0 -$P_cx_cornW 0 0 0
	
	for {set verAxis 2} {$verAxis <= $nIncCShape} {incr verAxis} {
		load [expr {$coreWallTag + ($nIncCShapeZ+1)*$verAxis}] 0 0 -$P_cx_field 0 0 0
	}
	
	load [expr {$coreWallTag + ($nIncCShapeZ+1)*($nIncCShape+1)}] 0 0 -$P_cx_cornE 0 0 0
}

puts $recInput "load [expr {$coreWallTag + ($nIncCShapeZ+1)}] 0 0 -$P_cx_cornW 0 0 0"
for {set verAxis 2} {$verAxis <= $nIncCShape} {incr verAxis} {
  puts $recInput "load [expr {$coreWallTag + ($nIncCShapeZ+1)*$verAxis}] 0 0 -$P_cx_field 0 0 0"
}
puts $recInput "load [expr {$coreWallTag + ($nIncCShapeZ+1)*($nIncCShape+1)}] 0 0 -$P_cx_cornE 0 0 0"
#